...
const Work = ({pushToWork, bunny}:workModuleInterface) => {
    ...
    return (
        <div className={'w-full h-full bg-[url(../public/images/fishbg.png)] bg-cover'}>
            <h2 className={'absolute top-[17vh] left-[13vw] text-white font-soyuz text-[5vh] leading-[100%] sm:top-[2vw] sm:text-[3vw] sm:left-[4vw]'}>Works</h2>
            <div className={'absolute top-[23vh] left-[13vw] w-[31vw] h-[3.5vh] border-2 border-white rounded-full sm:h-[2.5vw] sm:left-[4vw] sm:w-[70%] sm:top-[6vw]'}>
            ...
            <div className={'overflow-y-scroll absolute w-[90%] top-[30vh] h-[50vh] mx-[5%] sm:top-[10vw] sm:h-[35vw]'}>
                {workItems.map(item=>{
                    if(bunny.work?.find(index=>index.workItem.id==item.id)==undefined){
                        return(
                            <div key={item.id} className={'w-full my-[3.3vh] cursor-pointer sm:my-[4.5vw] sm:ml-[0%] sm:w-[95%]'} onClick={()=>{closePop();setWorkInPop(item)}}>
